...
2. Replace the data source URLs with your own sales data
...
The default decision threshold is **0.50**. At this threshold the model contacts approximately 45–60% of the customer base, capturing 57–69% of real buyers with a lift of **+15% to +27% over random outreach** depending on the run.

The more selective the model is allowed to be then fewer contacts, higher threshold — the larger the lift over random outreach becomes. Adjust `THRESHOLD` in both notebooks if your business context requires a different tradeoff between recall and precision.
...
- **Small dataset** — the Northwind dataset contains approximately 1,000 training samples, which limits model stability and statistical confidence of evaluation metrics.
- **Deep Learning variability** — due to the stochastic nature of neural network training, results vary between runs. The direction is consistent — the model always outperforms random outreach but the exact lift and contact volume depend on where the network converges. Re-exporting after retraining is recommended if the notebook is re-executed.
- **Short validation window** — the validation set covers approximately 4 months of data. Lift figures should be interpreted directionally rather than as precise production estimates.
